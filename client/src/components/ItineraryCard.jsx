        ...
        {posts.map((post) => (
          <div key={post.itinerary_title} className="flex flex-col rounded-lg shadow-lg overflow-hidden"> 
            <div className="flex-shrink-0">
              <img className="h-48 w-full object-cover" src={post.image ? post.image : placeholder_image} alt="destination" />
            </div>
            <div className="flex-1 bg-white p-6 flex flex-col justify-between">
              <div className="flex-1">
                ...
                <div className="block mt-2">
                  <p onClick={()=> navigate(`/library/${post.id}`)} className="text-xl font-semibold text-gray-900 hover:text-sky-600">{post.itinerary_title}</p>
                  <p className="mt-3 text-base text-gray-600">{post.itinerary_body}</p>
                </div>
              </div>
